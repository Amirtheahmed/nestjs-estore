...
export class CategoryService {
  ...
  async getCategories(
    ...
    const data = paginationResult.data.map((item) =>
      plainToInstance(CategoryOutputDto, item, {
  ...
  async getChildCategories(
    ...
    const immediateChildren = potentialImmediateChildren.filter(
      (child, index, array) => {
  ...
  async searchChildCategories(
    ...
    const immediateChildrenIds = potentialImmediateChildren
  ...
  async getParentCategory(subCategoryId: number): Promise<CategoryOutputDto> {
    ...
    if (!parentCategory) {
      throw new NotFoundException(
        `Parent category for subcategory ID ${subCategoryId} not found.`,
      );
    }

    return plainToInstance(CategoryOutputDto, parentCategory, {
      excludeExtraneousValues: true,
    });
  }
  ...
  async createCategory(dto: CreateCategoryDto): Promise<CategoryOutputDto> {
    ...
    return plainToInstance(CategoryOutputDto, newCategory, {
      excludeExtraneousValues: true,
    });
  }
  ...
  async deleteCategory(
    categoryId: number,
    deleteSubcategories: boolean = false,
  ): Promise<void> {
